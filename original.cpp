...
void measure_order_parameter(char *output);
...
int main(int argc, char **argv)
{
	...
	if(*choice == 'Q')
		{ measure_order_parameter(argv[10]);}	
...
double orderparameter_D2d()
{
	...
	double Q[3][3][3] = {{{0}}};
	int a, b, c, i;
	/** define Q_abc = < s (l_a m_b n_c + m_a l_b n_c) >/L3 **/
	for(a = 0; a < 3; a++)
		{	for(b = 0; b < 3; b++)
				{	for(c = 0; c < 3; c++)
						{	
							for(i = 0; i < L3; i++)
								{
									Q[a][b][c] += s[i]*(R[i][a]*R[i][3+b] + R[i][b]*R[i][3+a])*R[i][6+c];
									}	 
							}
					}
			}
...
void measure_order_parameter(char *output)
{
	double foo_B, Q1_B, Q2_B, chi_QB;
	double foo_n, Q1_n, Q2_n, chi_Qn;
	int i, j, site; 
...
//yacc = 0; yrej = 0; zacc = 0; zrej = 0;
		...
		/**** re-thermalization and reset Q1, Q2****/
		  ...
		  { for (i = 0; i < L3*4*tau ; i++)
			...
			foo_B = orderparameter_D2d();
			Q2_B += foo_B;
			Q1_B += sqrt(foo_B);
			
			foo_n = orderparameter_n();
			Q2_n += foo_n;
			Q1_n += sqrt(foo_n);
			 }	 
		...
		Q1_B /=sample_amount;
		Q2_B /= sample_amount;
		chi_QB = (Q2_B - Q1_B * Q1_B)*beta*L3;
		
		Q1_B /=sqrt(2); // nomalize Q1, but be careful. This should be after chi_Q
		
		Q1_n /=sample_amount;
		Q2_n /= sample_amount;
		chi_Qn = (Q2_n - Q1_n * Q1_n)*beta*L3;
		
		Q1_n /=sqrt(1.5); // nomalize Q1, but be careful. This should be after chi_Q

		output_file << 1/beta << '\t' << Q1_B << '\t' << chi_QB
					<< '\t' << Q1_n << '\t' << chi_Qn << endl;
